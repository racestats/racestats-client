...
SoftwareSerial serial(D7, D8, false, 256);

const unsigned char UBX_HEADER[]        = { 0xB5, 0x62 };
const unsigned char NAV_PVT_HEADER[]    = { 0x01, 0x07 };
const unsigned char TIM_TP_HEADER[]    = { 0x0D, 0x01 };
...
void gpsSendData(uint8_t *data, int size)
{
  ...
  while ( serial.available() ) {
    ...
    if ( fpos < 2 ) {
      ...
      if ( fpos == 4 ) {
        ...
        } else if ( compareMsgHeader(TIM_TP_HEADER) ) {
